...
namespace SyncBookPlayer.Extra;
...
sealed class SecondsToStringConverter : IValueConverter
{
	public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
	{
		...
		if (value is double doubleValue)
		{
			...
			if (timeSpan.Hours > 0 && timeSpan.Days < 1)
			{
				formatBuilder.Append(@"hh\:");
			}
			else if (timeSpan.Days > 0)
			{
                formatBuilder.Append(@"dd\:hh\:");
            }
		...
		{
			if (timespanValue.Hours > 0 && timespanValue.Days < 1)
			{
				formatBuilder.Append(@"hh\:");
			}
            else if (timespanValue.Days > 0)
            {
                formatBuilder.Append(@"dd\:hh\:");
            }
